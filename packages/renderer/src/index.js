...
// We must keep a single renderer instance, otherwise Vue will complain
let renderer;
...
const pdf = (initialValue) => {
  ...
  const updateContainer = (doc, callback) => {
    // The Vue reconciler builds the tree from mounted components (see <Document>);
    // hand-built element trees are lifted into the same internal format.
    container.document = doc ? ensureInstanceTree(doc) : doc;
    onChange();
