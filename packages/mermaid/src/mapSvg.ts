 ...
 * For the root `<svg>` element, width/height are replaced with the
 * user-provided size overrides.
 */
export function mapSvgNode(
  ...
  switch (type) {
    ...
    case P.Text:
      return h(Text, { key, ...mappedProps }, () => childElements);
    case P.Tspan:
      return h(Tspan, { key, ...mappedProps }, () => childElements);
    case P.Defs:
      return h(Defs, { key, ...mappedProps }, () => childElements);
    case P.Marker:
      return h(Marker, { key, ...mappedProps }, () => childElements);
    case P.TextInstance:
      return (node as any).value || null;
    default:
      console.log('Unsupported SVG element:', type);
